...
int chacha_test(void)
{
    ...
    static const byte cipher_big_result[] = {
            ...
            0x3a, 0x0f, 0x09, 0xe8, 0x00, 0xd1, 0x24, 0x97
    };

    static const byte cipher_big_result_1000[] = {
        0x5a, 0xca, 0x3f, 0x63, 0x71, 0xbf, 0xca, 0x99, 0x37, 0x9b,
        0x75, 0x97, 0x89, 0x10, 0x6e, 0xcf, 0xf2, 0xf5, 0xe3, 0xd5,
        0x45, 0x9b, 0xad, 0x10, 0x71, 0x6c, 0x5f, 0x6f, 0x7f, 0x22,
        0x77, 0x18, 0x2f, 0xf9, 0x99, 0xc5, 0x69, 0x58, 0x03, 0x12,
        0x86, 0x82, 0x3e, 0xbf, 0xc2, 0x12, 0x35, 0x43, 0xa3, 0xd9,
        0x18, 0x4f, 0x41, 0x11, 0x6b, 0xf3, 0x67, 0xaf, 0x3d, 0x78,
        0xe4, 0x22, 0x2d, 0xb3, 0x48, 0x43, 0x31, 0x1d, 0xef, 0xa8,
        0xba, 0x49, 0x8e, 0xa9, 0xa7, 0xb6, 0x18, 0x77, 0x84, 0xca,
        0xbd, 0xa2, 0x02, 0x1b, 0x6a, 0xf8, 0x5f, 0xda, 0xff, 0xcf,
        0x01, 0x6a, 0x86, 0x69, 0xa9, 0xe9, 0xcb, 0x60, 0x1e, 0x15
    };
    ...
    ret |= wc_Chacha_Process(&enc, cipher_big, plain_big,  sizeof(plain_big));
    ret |= wc_Chacha_Process(&dec, plain_big,  cipher_big, sizeof(cipher_big));
    if (ret != 0)
        return ret;

    if (XMEMCMP(plain_big, input_big, sizeof(cipher_big)))
        return -4330;

    // check if the first 100 bytes of cipher text are correct
    if (XMEMCMP(cipher_big, cipher_big_result, sizeof(cipher_big_result)))
        return -4340;

    // check if the first 100 bytes of cipher text after 1000 offset are correct
    if (XMEMCMP(cipher_big + 1000, cipher_big_result_1000, sizeof(cipher_big_result_1000)))
        return -4350;
